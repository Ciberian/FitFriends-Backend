...
import {
  ...
  TokenExpire,
  AuthErrorMessage,
  UnchangeableUserProperties,
  FavoriteGymsAction,
  DEFAULT_CLIENT_PROGRESS,
  USERS_COLLECTIONS_COUNT,
} from '../app.constant';
...
export class AuthService {
  ...
  public async register(dto: CreateClientDto | CreateTrainerDto) {
    const { email, name, password, gender, role, location } = dto;
    ...
    const trainerEntity = await new TrainerEntity(trainer).setPassword(password);
  ...
  public async refresh(id: Types.ObjectId, refreshToken: string) {
    const existUser =
  ...
  public async reqFriendship(
    ...
    const alert: IAlert = {
      text: `${requestor.name} ${(requestor.gender === UserGender.Female) ? 'отправила' : 'отправил'} вам запрос на добавление в друзья`,
      date: new Date(),
    };
  ...
  public async accFriendship({ userId: requestorId }: RequestToUserDto, respondentId: Types.ObjectId) {
    const respondent =
      (await this.clientRepository.findById(respondentId)) ||
      (await this.trainerRepository.findById(respondentId));
    ...
    const updatedRespondent = await this.quickUserUpdate(respondent._id, respondent.role, respondent);

    if (updatedRespondent) {
      const alert: IAlert = {
        text: `${respondent.name} ${(respondent.gender === UserGender.Female) ? 'добавила' : 'добавил'} вас в друзья`,
        date: new Date(),
      }
  ...
  public async rejFriendship({ userId: requestorId }: RequestToUserDto, respondentId: Types.ObjectId) {
    const requestor =
      (await this.clientRepository.findById(requestorId)) ||
      (await this.trainerRepository.findById(requestorId));
    ...
    if (!respondent.friends.find((friend) => friend.split('-')[0] === `${requestorId}`)) {
    ...
    if (updatedRespondent) {
      const alert: IAlert = {
        text: `${respondent.name} ${(respondent.gender === UserGender.Female) ? 'отклонила' : 'отклонил'} дружбу`,
        date: new Date(),
      }
  ...
  public async delFriend({ userId: friendId }: RequestToUserDto, userId: Types.ObjectId) {
    ...
    const friendIndex = userWhoDeletesFriend.friends.findIndex((friend) => friend.split('-')[0] === `${friendId}`);
    ...
    const updatedUser = await this.quickUserUpdate(userWhoDeletesFriend._id, userWhoDeletesFriend.role, userWhoDeletesFriend);

    if (updatedUser) {
      const userIndex = deletingFriend.friends.findIndex((friend) => friend.split('-')[0] === `${updatedUser._id}`);
      ...
      const alert: IAlert = {
        text: `${updatedUser.name} ${(updatedUser.gender === UserGender.Female) ? 'удалила' : 'удалил'} вас из списка друзей`,
        date: new Date(),
      }
      deletingFriend.alerts.push(alert);

      await this.quickUserUpdate(deletingFriend._id, deletingFriend.role, deletingFriend);
    }
  }
  ...
  public async reqPersonalTraining(
    ...
    if ((respondent.role === UserRole.Trainer) && !(respondent as ITrainer).personalTraining) {
      throw new ForbiddenException(AuthErrorMessage.NoPersonalTraining);
    }

    if ((respondent.role === UserRole.Client) && !(respondent as IClient).readyToTraining) {
      throw new ForbiddenException(AuthErrorMessage.NotReadyToTraining);
    }

    if (respondent.friends.includes(`${requestorId}-getReqTraining`) ||
    ...
    const alert: IAlert = {
      text: `${requestor.name} ${(requestor.gender === UserGender.Female) ? 'запросила' : 'запросил'} ${(respondent.role === UserRole.Trainer) ? 'персональную' : 'совместную'} тренировку`,
      date: new Date(),
    }
  ...
  public async accPersonalTraining({ userId: requestorId }: RequestToUserDto, respondentId: Types.ObjectId) {
    const respondent =
      (await this.clientRepository.findById(respondentId)) ||
      (await this.trainerRepository.findById(respondentId));
    ...
    const requestorIndex = respondent.friends.findIndex((friend) => friend.split('-')[0] ===`${requestorId}`);
    respondent.friends[requestorIndex] = `${requestorId}-accTraining`;
    const updatedRespondent = await this.quickUserUpdate(respondent._id, respondent.role, respondent);

    if (updatedRespondent) {
      const alert: IAlert = {
        text: `${respondent.name} ${(respondent.gender === UserGender.Female) ? 'приняла' : 'принял'} приглашение на ${(respondent.role === UserRole.Trainer) ? 'персональную' : 'совместную'} тренировку`,
        date: new Date(),
      }
      const respondentIndex = requestor.friends.findIndex((friend) => friend.split('-')[0] === `${respondentId}`);
  ...
  public async rejPersonalTraining({ userId: requestorId }: RequestToUserDto, respondentId: Types.ObjectId) {
    const requestor =
      (await this.clientRepository.findById(requestorId)) ||
      (await this.trainerRepository.findById(requestorId));
    ...
    if (updatedRespondent) {
      const alert: IAlert = {
        text: `${respondent.name} ${(respondent.gender === UserGender.Female) ? 'отклонила' : 'отклонил'} приглашение на ${(respondent.role === UserRole.Trainer) ? 'персональную' : 'совместную'} тренировку`,
        date: new Date(),
      }
      const respondentIndex = requestor.friends.findIndex((friend) => friend.split('-')[0] === `${respondentId}`);
  ...
  public async updateUser(id: Types.ObjectId, dto: UpdateClientDto | UpdateTrainerDto) {
    ...
    if (userBeforeUpdate.role === UserRole.Client) {
      ...
      const updatedClient = await this.clientRepository.update(id, clientEntity);
    ...
    const updatedTrainer = await this.trainerRepository.update(id, trainerEntity);
  ...
  private async createTokens(user: ClientEntity | TrainerEntity) {
    ...
    const access_token = await this.jwtService.signAsync(payload, {
      ...
      expiresIn: Number(this.configService.get('tokens.at_exp')) || TokenExpire.Access
    });

    const refresh_token = await this.jwtService.signAsync({}, {
      secret: this.configService.get('tokens.rt_secret'),
      expiresIn: Number(this.configService.get('tokens.rt_exp')) || TokenExpire.Refresh
    });

    return { access_token, refresh_token };
  }

  private async saveRefreshTokenHash(token: string, user: ClientEntity | TrainerEntity) {
    if (user.role === UserRole.Client) {
      const clientEntity = new ClientEntity(user as ClientEntity);
      await clientEntity.setRefreshToken(token);
      this.clientRepository.update(user._id, {refreshTokenHash: clientEntity.refreshTokenHash});
    } else {
      const trainerEntity = new TrainerEntity(user as TrainerEntity);
      await trainerEntity.setRefreshToken(token);
      this.trainerRepository.update(user._id, {refreshTokenHash: trainerEntity.refreshTokenHash});
    }
  }
  ...
  private async quickUserUpdate(
    ...
    if (userRole === UserRole.Client) {
      return this.clientRepository.update(userId, updatedData as Partial<ClientEntity>);
    } else {
      return this.trainerRepository.update(userId, updatedData as Partial<TrainerEntity>);
    }
  }
}
